...
recommenders paper (MovieLens ml-1m/ml-20m, KuaiRand Pure/1K), computed from the raw interaction logs on the pod
(groupby user -> #interactions, matching the HSTU preprocessor). Overlaid on log-x with the benchmark's SYNTHETIC
operating points (§5 zipf, §8 1B zipf/lognormal) so the reader can see the synthetic sequence lengths sit inside the
real range. Amazon (also used by the paper) is not downloaded here, so it's omitted. Data: runtime/realdata_seqlen.json
...
COL={"MovieLens ml-20m":"#e6194B","MovieLens ml-1m":"#f58231","KuaiRand-Pure":"#12a594","KuaiRand-1K":"#4363d8"}
...
ax.set_xscale("log"); ax.set_xlim(3,2e5)
...
ax.set_ylim(0,0.155)
ax.set_title("Real recsys per-user sequence lengths are heavy-tailed, spanning ~10 to 10^5\n"
