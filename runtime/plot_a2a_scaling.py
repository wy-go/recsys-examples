...
147-225, then 64 jumps back to 613. x is the TRUE GPU count (to scale, not categorical), so the 16→36 gap and the
64-GPU recovery land at their real positions and the non-monotonicity can't be an artifact of even spacing."""
import os
...
# shade the placement-noise region (>=36 GPU), starting midway in the 16->36 gap
ax.axvspan(26,68,color="#f0f0f0",zorder=0)
ax.text(50,690,"≥36 GPU: single runs — placement noise\n(same full-NVLink topology, 4× spread)",
        ha="center",va="top",fontsize=8.5,color="#888",style="italic")
ax.text(6.5,470,"8 / 16 GPU:\nreliable ~660 @512MB",ha="left",va="center",fontsize=8.5,color="#08519c")
...
ax.set_xlim(4,68)
ax.set_xlabel("GPUs in the NVLink supernode (x-axis to scale)",fontsize=11)
